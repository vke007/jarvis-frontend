...
SECRET_KEY = os.environ.get("SECRET_KEY", "jarvis-secret-key-change-in-railway")
...
def login():
    ...
    if email == OWNER_EMAIL.lower():
        if not OWNER_PASSWORD:
            return jsonify({"error": "⚠️ OWNER_PASSWORD not set in Railway Variables"}), 500
...
def dashboard():
    ...
    h = HealthLog.query.filter_by(owner=request.email, log_date=today).first()
    return jsonify({
        "pending_tasks": Task.query.filter_by(owner=request.email, completed=False).count(),
...
@app.get("/")
def root():
    return jsonify({"msg": "JARVIS API v3 - All Features Unlocked"})
...
def init_db():
    with app.app_context():
        ...
        db.session.commit()
    print("✅  All features unlocked — JARVIS v3 (Python 3.12.10)")
